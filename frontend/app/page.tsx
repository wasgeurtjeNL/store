...
interface MedusaProductListItem {
  ...
  variants: {
    calculated_price?: { // Expect calculated_price here too based on /producten page
...
const mockProductsData: ProductsByCategory = {
  'best-verkocht': [
    { id: 'prod_1', name: 'Wasparfum proefpakket', price: '14,95', imageUrl: '/images/Wasparfum-proefpakket.webp', altText: 'Wasparfum proefpakket' },
    { id: 'prod_2', name: 'Wasgeurtje Wasstrips', price: '14,95', imageUrl: '/images/Wasgeurtje-Wasstrips.webp', altText: 'Wasgeurtje Wasstrips' },
    { id: 'prod_3', name: 'Blossom Drip', price: '14,95', imageUrl: '/images/Blossom-Drip.webp', altText: 'Wasparfum Blossom Drip' },
    { id: 'prod_4_mock', name: 'Full Moon (Mock)', price: '14,95', imageUrl: '/images/Full-Moon.webp', altText: 'Wasparfum Full Moon' }, // Update ID om conflicten te voorkomen
  ],
  premium: [
    { id: 'prod_5', name: 'White Musk', price: '15,95', imageUrl: '/images/White-Musk.webp', altText: 'Premium Wasparfum White Musk' },
    { id: 'prod_6', name: 'Evening Dew', price: '15,95', imageUrl: '/images/Evening-Dew.webp', altText: 'Premium Wasparfum Evening Dew' },
    { id: 'prod_7', name: 'Ylang Scent', price: '15,95', imageUrl: '/images/Ylang-Scent.webp', altText: 'Premium Wasparfum Ylang Scent' },
    { id: 'prod_8', name: 'Flower Rain', price: '15,95', imageUrl: '/images/Flower-Rain.webp', altText: 'Premium Wasparfum Flower Rain' },
  ],
  'collecties-sets': [
    { id: 'prod_9', name: 'Cadeauset wasparfum', price: '74,95', imageUrl: '/images/Cadeauset-wasparfum.webp', altText: 'Cadeauset wasparfum' },
    { id: 'prod_1b', name: 'Wasparfum proefpakket', price: '14,95', imageUrl: '/images/Wasparfum-proefpakket.webp', altText: 'Wasparfum proefpakket' },
    { id: 'prod_10', name: 'Nieuwe geurcollectie proefpakket', price: '16,95', imageUrl: '/images/Nieuwe-geurcollectie-proefpakket.webp', altText: 'Nieuwe geurcollectie proefpakket' },
    { id: 'prod_4b_mock', name: 'Full Moon (Mock Set)', price: '14,95', imageUrl: '/images/Full-Moon.webp', altText: 'Wasparfum Full Moon' }, // Update ID
  ],
};

export const metadata: Metadata = {
  title: "Luxe Wasparfum kopen?",
  description: "Bestel luxe wasparfum op Wasgeurtje.nl. Italiaans geïnspireerde geuren, veilig voor alle stoffen en geliefd bij 1400+ klanten.",
};

// Fetch function for single product (basic info only)
async function getSingleProduct(productId: string): Promise<MedusaSingleProduct | null> {
  // Skip API calls if we're in development and the debug flag is set
  const debugMode = process.env.NEXT_PUBLIC_DEBUG_MODE === 'true';
  const isDev = process.env.NODE_ENV === 'development';
  
  if (debugMode && isDev) {
    console.log(`[getSingleProduct] Skipping API call in debug mode for product ${productId}`);
    return null;
  }
  ...
  try {
    ...
    try {
      ...
      if (!response.ok) {
        console.error(`[getSingleProduct] HTTP error! status: ${response.status}, body: ${await response.text()}`);
        return null;
      }
      ...
      console.error(`[getSingleProduct] Fetch operation failed for ${productId}:`, fetchError);
      return null;
    }
  } catch (error) {
...
async function getProductListWithPrices(regionId: string): Promise<MedusaProductListResponse | null> {
  // Skip API calls if we're in development and the debug flag is set
  const debugMode = process.env.NEXT_PUBLIC_DEBUG_MODE === 'true';
  const isDev = process.env.NODE_ENV === 'development';
  ...
  try {
    ...
    try {
      ...
      if (!response.ok) {
         console.error(`[getProductListWithPrices] HTTP error! status: ${response.status}, body: ${await response.text()}`);
        return null;
      }
      
      const data: MedusaProductListResponse = await response.json();
      console.log(`[getProductListWithPrices] Success, found ${data.products?.length || 0} products.`);
      return data;
    } catch (fetchError) {
      console.error("[getProductListWithPrices] Fetch operation failed:", fetchError);
      return null;
    }
  } catch (error) {
...
export default async function Home() {
  ...
  let productsData = JSON.parse(JSON.stringify(mockProductsData));
  ...
  try {
    ...
    apiResults.forEach((result, index) => {
      if (result.status === 'rejected') {
        const productName = ['Full Moon', 'Blossom Drip', 'Wasstrips', 'Proefpakket', 'Cadeauset', 'Product List'][index];
  ...
  const fullMoonSingleData = fullMoonResult.status === 'fulfilled' ? fullMoonResult.value : null;
  const blossomDripSingleData = blossomDripResult.status === 'fulfilled' ? blossomDripResult.value : null;
  const wasstripsSingleData = wasstripsResult.status === 'fulfilled' ? wasstripsResult.value : null;
  const proefpakketSingleData = proefpakketResult.status === 'fulfilled' ? proefpakketResult.value : null;
  const cadeausetSingleData = cadeausetResult.status === 'fulfilled' ? cadeausetResult.value : null;
  const productListData = productListResult.status === 'fulfilled' ? productListResult.value : null;

  // Process Full Moon
  let fullMoonProduct: Product | null = null;
  if (fullMoonSingleData) {
    const productFromList = productListData?.products.find(p => p.id === fullMoonProductId);
    ...
    if (calculatedPrice) {
      ...
      console.warn(`Price not found for Full Moon (${fullMoonProductId}) in the product list response.`);
    }
  }
  ...
  if (blossomDripSingleData) {
    const productFromList = productListData?.products.find(p => p.id === blossomDripProductId);
    const calculatedPrice = productFromList?.variants?.[0]?.calculated_price;
    if (calculatedPrice) {
      blossomDripProduct = {
      ...
      console.warn(`Price not found for Blossom Drip (${blossomDripProductId}) in the product list response.`);
    }
  }
  ...
  if (wasstripsSingleData) {
    const productFromList = productListData?.products.find(p => p.id === wasstripsProductId);
    const calculatedPrice = productFromList?.variants?.[0]?.calculated_price;
    if (calculatedPrice) {
      wasstripsProduct = {
      ...
      console.warn(`Price not found for Wasstrips (${wasstripsProductId}) in the product list response.`);
    }
  }
  ...
  if (proefpakketSingleData) {
    const productFromList = productListData?.products.find(p => p.id === proefpakketProductId);
    const calculatedPrice = productFromList?.variants?.[0]?.calculated_price;
    if (calculatedPrice) {
      proefpakketProduct = {
      ...
      console.warn(`Price not found for Proefpakket (${proefpakketProductId}) in the product list response.`);
    }
  }
  ...
  if (cadeausetSingleData) {
    const productFromList = productListData?.products.find(p => p.id === cadeausetProductId);
    const calculatedPrice = productFromList?.variants?.[0]?.calculated_price;
    if (calculatedPrice) {
      cadeausetProduct = {
      ...
      console.warn(`Price not found for Cadeauset (${cadeausetProductId}) in the product list response.`);
    }
  }

  // Replace mock data if we successfully constructed the products
  if (fullMoonProduct) {
    const indexToReplace = productsData['best-verkocht'].findIndex((p: Product) => p.id === mockFullMoonId);
    if (indexToReplace !== -1) {
      console.log(`Replacing mock Full Moon at index ${indexToReplace} with combined data.`);
      productsData['best-verkocht'][indexToReplace] = fullMoonProduct;
    } else {
      console.warn(`Mock Full Moon with ID '${mockFullMoonId}' not found. Adding combined product.`);
      productsData['best-verkocht'].push(fullMoonProduct);
    }
  } else {
    console.warn(`Failed to construct full product data for Full Moon (${fullMoonProductId}). Using mock data.`);
  }

  if (blossomDripProduct) {
    const indexToReplace = productsData['best-verkocht'].findIndex((p: Product) => p.id === mockBlossomDripId);
    if (indexToReplace !== -1) {
      console.log(`Replacing mock Blossom Drip at index ${indexToReplace} with combined data.`);
      productsData['best-verkocht'][indexToReplace] = blossomDripProduct;
    } else {
      console.warn(`Mock Blossom Drip with ID '${mockBlossomDripId}' not found. Adding combined product.`);
      productsData['best-verkocht'].push(blossomDripProduct);
    }
  } else {
    console.warn(`Failed to construct full product data for Blossom Drip (${blossomDripProductId}). Using mock data.`);
  }

  if (wasstripsProduct) {
    const indexToReplace = productsData['best-verkocht'].findIndex((p: Product) => p.id === mockWasstripsId);
    if (indexToReplace !== -1) {
      console.log(`Replacing mock Wasstrips at index ${indexToReplace} with combined data.`);
      productsData['best-verkocht'][indexToReplace] = wasstripsProduct;
    } else {
      console.warn(`Mock Wasstrips with ID '${mockWasstripsId}' not found. Adding combined product.`);
      productsData['best-verkocht'].push(wasstripsProduct);
    }
  } else {
    console.warn(`Failed to construct full product data for Wasstrips (${wasstripsProductId}). Using mock data.`);
  }

  if (proefpakketProduct) {
    const indexToReplace = productsData['best-verkocht'].findIndex((p: Product) => p.id === mockProefpakketId);
    if (indexToReplace !== -1) {
      console.log(`Replacing mock Proefpakket at index ${indexToReplace} with combined data.`);
      productsData['best-verkocht'][indexToReplace] = proefpakketProduct;
    } else {
      console.warn(`Mock Proefpakket with ID '${mockProefpakketId}' not found. Adding combined product.`);
      productsData['best-verkocht'].push(proefpakketProduct);
    }
  } else {
    console.warn(`Failed to construct full product data for Proefpakket (${proefpakketProductId}). Using mock data.`);
  }
  ...
  if (cadeausetProduct) {
    ...
    const bestVerkIndex = productsData['best-verkocht'].findIndex((p: Product) => p.id === mockCadeausetId || p.id === cadeausetProductId);
    if (bestVerkIndex !== -1) {
    ...
    const collectieIndex = productsData['collecties-sets'].findIndex((p: Product) => p.id === mockCadeausetId || p.id === cadeausetProductId);
    if (collectieIndex !== -1) {
      console.log(`Removing Cadeauset from collecties-sets category.`);
      productsData['collecties-sets'].splice(collectieIndex, 1);
    }
  } else {
    console.warn(`Failed to construct full product data for Cadeauset (${cadeausetProductId}). Using mock data.`);
    
    // If we don't have real data, also remove the mock cadeauset from all product grids
    const mockBestVerkIndex = productsData['best-verkocht'].findIndex((p: Product) => p.id === mockCadeausetId);
    ...
    const mockCollectieIndex = productsData['collecties-sets'].findIndex((p: Product) => p.id === mockCadeausetId);
  ...
  return (
    <>
      ...
      <div className="bg-[#1a1a1a] text-white text-center py-2 w-full relative overflow-hidden group">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-[#1a1a1a]/0 via-[#333]/20 to-[#1a1a1a]/0 opacity-0 group-hover:opacity-100 transition-opacity duration-500" style={{ backgroundSize: '200% 100%', animation: 'shimmer 3s ease-in-out infinite' }}></div>
        
        {/* Left sparkle */}
        <div className="absolute left-[15%] top-1/2 transform -translate-y-1/2 w-1 h-1 bg-white rounded-full opacity-0 group-hover:opacity-60" style={{ animation: 'ping 2s ease-in-out infinite' }}></div>
        
        {/* Right sparkle */}
        <div className="absolute right-[15%] top-1/2 transform -translate-y-1/2 w-1 h-1 bg-white rounded-full opacity-0 group-hover:opacity-60" style={{ animation: 'ping 2s ease-in-out 0.5s infinite' }}></div>
        
        <span className="text-sm relative inline-flex items-center">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4 mr-2 text-[#dba94a]">
      ...
      <div className="bg-black text-white text-center py-2 w-full border-t border-[#333333] relative overflow-hidden group">
        ...
        <div className="absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-[#dba94a] via-[#f8d570] to-[#dba94a] opacity-0 group-hover:opacity-100 transition-opacity duration-500" style={{ backgroundSize: '200% 100%', animation: 'shimmer 3s ease-in-out infinite' }}></div>
      ...
      <main className="flex flex-col min-h-screen">
              ...
              <div className="w-1/2 bg-gradient-to-r from-[#dba94a] via-[#f8d570] to-[#fef9ec] p-12 lg:p-16 flex flex-col justify-center relative z-10">
                ...
                <div className="bg-white/90 rounded-full px-4 py-1.5 inline-flex items-center w-fit mb-8 shadow-sm hover:shadow-md hover:bg-white transition-all duration-300 cursor-pointer">
                  <div className="flex mr-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <span key={star} className="text-[#dba94a] hover:text-[#FCCE4E] transition-colors duration-200">★</span>
                    ))}
                  </div>
                  <span className="text-gray-800 text-sm font-medium">4.8/5 (1400+ google reviews)</span>
                </div>
                
                {/* Heading */}
                <h1 className="text-[2.75rem] md:text-5xl lg:text-[3.5rem] xl:text-[4rem] font-bold text-gray-900 mb-7 leading-[1.15]">
                  <span className="block hover:text-[#814E1E] transition-colors duration-300">Luxe wasparfums die</span> 
                  <span className="block hover:text-[#814E1E] transition-colors duration-300">uitzonderlijk lang blijven</span>
                  <span className="block hover:text-[#814E1E] transition-colors duration-300">hangen</span>
                </h1>
                
                {/* Subheading */}
                <p className="text-lg text-gray-800 mb-9 max-w-md hover:text-[#814E1E] transition-colors duration-300">
                  Italiaans geïnspireerde geuren gemaakt met eersteklas essentiële oliën
                </p>
                ...
                <div className="max-w-[250px]">
                  ...
                  >
                    <span className="relative z-10 group-hover:tracking-wider transition-all duration-300">ONTDEK NU</span>
            ...
            <div className="block md:hidden">
              ...
              <div className="bg-gradient-to-r from-[#dba94a] via-[#f8d570] to-[#fef9ec] p-6">
                ...
                <div className="bg-white/90 rounded-full px-3 py-.5 inline-flex items-center w-fit mb-4 shadow-sm">
                  <div className="flex mr-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <span key={star} className="text-[#dba94a]">★</span>
                    ))}
                  </div>
                  <span className="text-gray-800 text-xs font-medium">4.8/5 (1400+ reviews)</span>
                </div>
          ...
          <div className="container mx-auto px-4 relative z-10 pt-2">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10">
              ...
              <div className="text-center group">
                ...
                <h3 className="font-bold text-gray-900 text-lg mb-0 group-hover:text-[#814E1E] transition-colors duration-300">Veilig voor alle stoffen en</h3>
                <p className="text-gray-800 text-base group-hover:text-[#814E1E] transition-colors duration-300">machines</p>
              </div>
              ...
              <div className="text-center group">
                ...
                <h3 className="font-bold text-gray-900 text-lg mb-0 group-hover:text-[#814E1E] transition-colors duration-300">Geformuleerd voor de gevoelige</h3>
                <p className="text-gray-800 text-base group-hover:text-[#814E1E] transition-colors duration-300">huid</p>
              </div>
              ...
              <div className="text-center group">
                ...
                <h3 className="font-bold text-gray-900 text-lg mb-0 group-hover:text-[#814E1E] transition-colors duration-300">Milieuvriendelijk &</h3>
                <p className="text-gray-800 text-base group-hover:text-[#814E1E] transition-colors duration-300">Parabenenvrij</p>
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-6 md:gap-10 mb-12">
              ...
              <div className="group cursor-pointer">
                <div className="h-48 md:h-64 relative rounded-lg overflow-hidden mb-4 md:mb-6 shadow-md group-hover:shadow-xl group-hover:shadow-[#d6ad61]/20 transition-all duration-300">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 text-white p-4 transition-all duration-300 translate-y-4 group-hover:translate-y-0">
                    <p className="text-sm">Bekijk onze bestverkochte wasparfums</p>
                </div>
                </div>
                <h3 className="font-bold text-lg md:text-xl lg:text-2xl text-gray-900 text-center group-hover:text-[#814E1E] transition-colors duration-300">Best Verkocht</h3>
              </div>
              ...
              <div className="group cursor-pointer">
                ...
                <h3 className="font-bold text-lg md:text-xl lg:text-2xl text-gray-900 text-center group-hover:text-[#814E1E] transition-colors duration-300">Premium Wasparfums</h3>
              </div>
              ...
              <div className="group cursor-pointer">
                <div className="h-48 md:h-64 relative rounded-lg overflow-hidden mb-4 md:mb-6 shadow-md group-hover:shadow-xl group-hover:shadow-[#d6ad61]/20 transition-all duration-300">
                  ...
                  />
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 text-white p-4 transition-all duration-300 translate-y-4 group-hover:translate-y-0">
                    <p className="text-sm">Probeer verschillende geuren met ons proefpakket</p>
                </div>
                </div>
                <h3 className="font-bold text-lg md:text-xl lg:text-2xl text-gray-900 text-center group-hover:text-[#814E1E] transition-colors duration-300">Proefpakket</h3>
              </div>
              ...
              <div className="group cursor-pointer">
                <div className="h-48 md:h-64 relative rounded-lg overflow-hidden mb-4 md:mb-6 shadow-md group-hover:shadow-xl group-hover:shadow-[#d6ad61]/20 transition-all duration-300">
                  ...
                  />
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 text-white p-4 transition-all duration-300 translate-y-4 group-hover:translate-y-0">
                    <p className="text-sm">Het perfecte cadeau voor elke gelegenheid</p>
                </div>
                </div>
                <h3 className="font-bold text-lg md:text-xl lg:text-2xl text-gray-900 text-center group-hover:text-[#814E1E] transition-colors duration-300">Geschenkset</h3>
              </div>
            </div>
        ...
        <section className="bg-[#814E1E] py-5"> {/* Increased vertical padding */}
          <div className="container mx-auto px-4">
            <div className="flex flex-wrap justify-center items-center gap-x-8 gap-y-4 md:gap-x-16 lg:gap-x-20 text-white"> {/* Increased gaps */}
              {/* USP 1 - Geen parabenen */}
              <div className="flex items-center gap-3.5"> {/* Increased gap between icon and text */}
                <div className="w-14 h-14 bg-[#814E1E]/50 rounded-full flex items-center justify-center"> {/* Larger icon with background */}
                  <Image 
                ...
                <span className="text-base md:text-lg font-medium">Geen parabenen</span>
              </div>
              ...
              <div className="flex items-center gap-3.5">
                ...
                <span className="text-base md:text-lg font-medium">Geen microplastics</span>
              </div>
              ...
              <div className="flex items-center gap-3.5">
                ...
                <span className="text-base md:text-lg font-medium">Biologisch afbreekbaar</span>
              </div>
              ...
              <div className="flex items-center gap-3.5">
                ...
                <span className="text-base md:text-lg font-medium">100% Veganistisch <br className="hidden sm:block" />& Dierproefvrij</span>
              </div>
            </div>
          </div>
        </section>
   {/* Section under the brown bar */}
            ...
            <div className="max-w-7xl mx-auto pb-20">
              <div className="flex flex-col md:flex-row justify-between items-start">
                ...
                <div className="flex flex-col items-center md:w-1/3 px-4 mb-12 md:mb-0">
                  ...
                  Giet een dopje van Wasgeurtje's geconcentreerde wasparfum in het wasverzachtervakje van je wasmachine. Een kleine hoeveelheid is al voldoende voor een heerlijke, langdurige geur.
                </p>
              </div>

                {/* Arrow 1 - only visible on desktop */}
                <div className="hidden md:flex items-center justify-center md:w-auto">
                  <svg xmlns="http://www.w3.org/2000/svg" width="35" height="25" viewBox="0 0 35 25" fill="none" className="text-[#D6AD61] transform -translate-y-20">
                    <path d="M34.4596 13.6168C35.1553 12.9211 35.1553 11.7964 34.4596 11.1008L23.9138 0.554918C23.2181 -0.140744 22.0934 -0.140744 21.3978 0.554918C20.7021 1.25058 20.7021 2.37531 21.3978 3.07097L30.6856 12.3588L21.3978 21.6466C20.7021 22.3422 20.7021 23.467 21.3978 24.1626C22.0934 24.8583 23.2181 24.8583 23.9138 24.1626L34.4596 13.6168ZM0.708496 14.1142H33.0996V10.6034H0.708496V14.1142Z" fill="#D6AD61"/>
                    </svg>
              </div>
                ...
                <div className="flex flex-col items-center md:w-1/3 px-4 mb-12 md:mb-0">
                  ...
                  <p className="text-[16px] text-center font-normal text-[#212529] leading-[150%] max-w-[280px]">
                  Onze formule is geschikt voor alle soorten textiel en wasmachines. Was zoals je gewend bent en geniet van een geur die niet alleen fris blijft, maar ook zacht is voor je kleding.
                </p>
              </div>
                ...
                <div className="flex flex-col items-center md:w-1/3 px-4">
                  ...
                  <p className="text-[16px] text-center font-normal text-[#212529] leading-[150%] max-w-[280px]">
                  Haal je was uit de wasmachine en ervaar een langdurige, luxe geur geïnspireerd door Italiaanse parfums. Jouw kleding blijft fris en heerlijk geurig, wasbeurt na wasbeurt.
                </p>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-16">
                ...
                <div className="flex flex-col items-center justify-center py-2 gap-1">
                  <Image 
                    ...
                    height={42} 
                  />
                  <p className="text-[16px] font-medium text-[#212529] text-center">Veilig voor alle textiel en wasmachines</p>
                  </div>
                ...
                <div className="flex flex-col items-center justify-center py-2 gap-1">
                  <Image 
                    ...
                    height={42} 
                  />
                  <p className="text-[16px] font-medium text-[#212529] text-center">Werkt met elke wasmachine</p>
                </div>
                ...
                <div className="flex flex-col items-center justify-center py-2 gap-1">
                  <Image 
                    ...
                    height={42} 
                  />
                  <p className="text-[16px] font-medium text-[#212529] text-center">Geformuleerd voor de gevoelige huid</p>
                  </div>
                </div>
            </div>
          </div>
        ...
        <section className="py-16 bg-[#fef9ec]"> {/* Achtergrondkleur zoals andere lichte secties */}
          <div className="container mx-auto px-4 text-center">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-12 md:gap-x-8">
              ...
              <div className="flex flex-col items-center group">
                ...
                <p className="text-sm md:text-base text-[#212529] font-normal leading-5 md:leading-6 text-center transition-all duration-300 group-hover:text-[#814E1E] group-hover:font-medium">
                  100% Veganistisch<br />& Dierproefvrij
                </p>
              </div>
              ...
              <div className="flex flex-col items-center group">
                ...
                <p className="text-sm md:text-base text-[#212529] font-normal leading-5 md:leading-6 text-center transition-all duration-300 group-hover:text-[#814E1E] group-hover:font-medium">
                  Flessen van<br />gerecycled plastic
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-lg shadow-xl overflow-hidden md:flex md:items-center border border-gray-200 relative">
              ...
              <div className="md:w-7/12 p-8 lg:p-10 flex flex-col">
                ...
                <p className="text-base text-gray-700 mb-6">
                  Twijfel je nog tussen geuren? Kies niet één, maar allemaal – met deze <span className="font-medium text-gray-800">complete set</span> van onze <span className="font-medium text-gray-800">bestverkochte</span> wasparfums.
                </p>
                
                {/* Product details */}
                <div className="mb-6 space-y-3 border-b border-gray-200 pb-6">
                  <div className="flex items-start">
                    <span className="text-sm font-semibold text-[#814E1E] uppercase tracking-wider w-28 flex-shrink-0">INCLUDES:</span>
                    <p className="text-base text-gray-800">
                       <span className="text-[#D6AD61] mr-1">✓</span> x5 10ml flesjes
                    </p>
                  </div>
                  <div className="flex items-start">
                    <span className="text-sm font-semibold text-[#814E1E] uppercase tracking-wider w-28 flex-shrink-0">PERFUMES:</span>
                    <div>
                      <span className="font-medium text-gray-800 mr-2">• Morning Vapor</span> 
                      <span className="font-medium text-gray-800 mr-2">• Flower Rain</span> 
                      <span className="font-medium text-gray-800 mr-2">• Blossom Drip</span> 
                      <span className="font-medium text-gray-800 mr-2">• Sundance</span> 
                      <span className="font-medium text-gray-800">• Full Moon</span>
                    </div>
                  </div>
                </div>
                 
                <p className="text-xs text-gray-500 mb-8 italic">
                  *2-4 wasbeurten per flesje, afhankelijk van de gewenste geursterkte.*
                </p>
                ...
                <div className="mt-auto">
                  ...
                  >
                    ...
                    <div className="flex items-center justify-center mt-1">
                      <span className="mr-2 text-gray-700 line-through text-sm">€93,65</span>
                      <span className="font-bold text-xl text-black">€14,95</span>
                    </div>
                  </a>
                </div>
              </div>
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "@type": "Product",
              "name": cadeausetProduct?.name || "Bestverkochte wasgeuren als voordeelpakket",
              "image": [
                cadeausetProduct?.imageUrl || "/images/Cadeauset-wasparfum.webp"
              ],
              "description": "Luxe cadeauset met bestverkochte wasparfums. Italiaans geïnspireerde geuren met langdurig effect.",
              "brand": {
                "@type": "Brand",
                "name": "Wasgeurtje.nl"
              },
