...
const Testimonials = () => {
  ...
  const renderStars = (rating: number) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(
        <svg key={`star-${i}`} className="w-5 h-5 text-yellow-400 fill-current" viewBox="0 0 24 24">
    ...
    if (hasHalfStar) {
      stars.push(
        <svg key="half-star" className="w-5 h-5 text-yellow-400 fill-current" viewBox="0 0 24 24">
          <path
    ...
    for (let i = 0; i < emptyStars; i++) {
      stars.push(
        <svg key={`empty-star-${i}`} className="w-5 h-5 text-gray-300 fill-current" viewBox="0 0 24 24">
          <path d="M12 17.27L18.18 21L16.54 13.97L22 9.24L14.81 8.63L12 2L9.19 8.63L2 9.24L7.46 13.97L5.82 21L12 17.27Z" />
        </svg>,
      )
    }
            ...
            {testimonials.map((testimonial) => (
              <SwiperSlide key={testimonial.id}>
                <div className="bg-white p-6 rounded-lg shadow-sm h-full flex flex-col">
                  ...
                  <button className="text-gray-500 text-sm mt-4 self-start hover:underline">Lees verder</button>
                </div>
              </SwiperSlide>
            ))}
          </Swiper>
